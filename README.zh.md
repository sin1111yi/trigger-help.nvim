...
零依赖（picker 需要 snacks.nvim）、模块精简（`lua/trigger_help/`）、`setup()` 全参数可配。
...
-- 插件 spec
{
  ...
  lazy = false, -- eager：:TriggerHelp 应立即可用
  config = function()
...
当 `~/Development/trigger-help.nvim` 存在时用它；其他没有该目录的机器自动回退 GitHub 安装。
...
```
lua/trigger_help/
  init.lua     入口：setup、:TriggerHelp、API
  config.lua   配置项
  lang.lua     语言（LC_ALL > LC_MESSAGES > LANG）、双语选择
  source.lua   注册 + 内容 + help tag 扫描 + 名称解析
  panel.lua    面板窗口 / 渲染
  picker.lua   snacks picker 集成
```
